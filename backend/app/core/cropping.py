...
def auto_crop_passport(
    ...
    full_head_height = face_h * 1.0
    target_head_height = int(target_height * head_height_percent * 0.75)
    scale = target_head_height / full_head_height

    scaled_img = image.resize((int(iw * scale), int(ih * scale)), Image.LANCZOS)
    scaled_w, scaled_h = scaled_img.size

    eye_y_orig = face_y + face_h * 0.35
    face_cx_orig = face_x + face_w / 2.0

    eye_y_scaled = eye_y_orig * scale
    face_cx_scaled = face_cx_orig * scale

    # eye_level_percent = fraction from BOTTOM where eyes should appear.
    # Convert to pixels from the TOP of the output image.
